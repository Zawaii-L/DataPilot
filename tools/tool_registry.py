...
def create_default_tool_registry() -> ToolRegistry:
    ...
    from tools.business.data.semantic_data_tools import (
    ...
    from tools.business.data.dataframe_tools import (
        create_dataframe,
        build_dataframe,
    )
    from tools.business.data.regression_tools import (
        regression_analysis,
        regression_predict,
    )
    from tools.business.data.regression_visualization import (
        generate_regression_visualizations,
    )
    from tools.business.web.web_data_tools import (
        download_data_file,
    ...
    from tools.business.document.document_report_tools import (
        generate_document_summary_report,
    )
    from tools.business.office.word_edit_tools import apply_word_edits
    from tools.business.office.excel_edit_tools import apply_excel_edits
    from tools.business.office.excel_report_tools import (
        create_professional_excel_report,
        inspect_professional_excel_report,
    )
    from tools.business.office.professional_word_report_tools import (
    ...
    registry.register(
        ...
        create_dataframe,
        (
            "将列表、字典、JSON结构数据转换为标准 pandas DataFrame。"
            "适用于网页搜索结果、API返回数据、LLM结构化数据进入 DataPilot 数据分析流水线。"
        ),
        category="data",
        parameters={
            "data": "需要转换的数据。支持 list、dict、JSON结构或已有 DataFrame。",
            "columns": "二维列表数据对应的字段名称列表，可选。",
        },
    ...
    registry.register(
        ...
        build_dataframe,
        (
            "将非标准结构化数据转换为标准 pandas DataFrame，"
            "并返回基础数据质量摘要。用于 Agent 从互联网获取商业数据后，"
            "进入统计分析、机器学习和办公报告生成流程。"
        ),
        category="data",
        parameters={
            "data": "列表、字典、JSON结构数据或已有 DataFrame。",
            "columns": "二维列表字段名称，可选。",
        },
        returns="包含 dataframe 和 summary 的结果字典",
        aliases=[
    ...
    registry.register(
        ...
        regression_analysis,
        (
            "对 pandas DataFrame 执行线性回归分析。"
            "用于商业问题中的影响因素分析、变量关系建模、"
            "趋势解释和预测建模。"
        ),
        category="analysis",
        parameters={
            "df": "pandas DataFrame",
            "target_column": "需要预测的目标字段，例如销量、销售额、价格",
            "feature_columns": "影响因素字段列表，可选；为空时自动选择数值字段",
        },
        returns=(
            "包含模型类型、R²、均方误差、截距、"
            "变量系数、回归公式和样本量的分析结果"
    ...
    registry.register(
        ...
        regression_predict,
        (
            "根据 regression_analysis 输出的模型参数进行预测。"
            "用于商业场景中的未来值估计。"
        ),
        category="analysis",
        parameters={
            "model_result": "regression_analysis 返回结果",
            "input_data": "预测输入变量字典",
        },
        returns="预测结果数值",
        aliases=[
    ...
    registry.register(
        ...
        (
            ...
            "包括实际值与预测值对比图、"
            "回归因素影响图和回归关系散点图。"
            "用于 Excel、Word 商业报告中的图表生成。"
        ),
        category="analysis",
        parameters={
            "df": "pandas DataFrame",
            "target_column": "目标变量字段，例如销量、销售额",
            "feature_columns": "影响因素字段列表",
            "coefficients": "regression_analysis 返回的变量系数字典",
            "output_dir": "PNG 图表输出目录",
        },
        returns=(
            "生成的 PNG 文件路径字典，"
            "包含预测对比图、因素影响图、关系散点图"
    ...
    registry.register(
        ...
        download_data_file,
        (
            "从明确 URL 下载 CSV / Excel 等结构化数据文件到本地目录。"
            "若服务器实际返回 text/plain / Markdown，会保留真实文本扩展名，"
            "后续应使用 read_document，而不是把文本强行按 CSV 解析。"
        ),
        category="web",
        parameters={
            "url": "需要下载的 URL",
    ...
    registry.register(
        ...
        download_document_file,
        (
            "从明确 URL 下载 TXT / Markdown / PDF / DOCX 等文档来源。"
            "适合 raw text、报告 PDF、Markdown 原始资料等不能由 read_webpage "
            "直接读取的来源；下载后应调用 read_document 获取正文证据。"
        ),
        category="web",
        parameters={
            "url": "需要下载的完整 http/https URL",
            "output_dir": "下载目录",
