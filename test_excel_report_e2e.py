...
class FakeTaskPlanner:
    ...
    -> Professional Excel Tool -> Verification Engine -> Completion Gate。
    """
    ...
    def create_plan(self, task, context=None):
        return TaskPlan(
            ...
            execution_requirements=[
                "读取真实销售 Excel。",
                "按城市统计销售额合计。",
                "生成包含 KPI 和城市销售额对比图表的专业 Excel 报告。",
            ],
            verification_requirements=[
                "生成后重新读取并检查最终 Excel 文件。",
            ],
            safety_requirements=[
                "不得覆盖源 Excel。",
            ],
...
class FakeCompletions:
    ...
    注意：
    - 真正的数据读取、分组、排序、Excel 生成和 Excel 检查全部由
    ...
    def create(self, **kwargs):
        ...
        decisions = [
            ...
            },
            {
                ...
                "purpose": (
                    "把真实分析结果生成带 KPI、专业格式和原生图表的最终 Excel。"
                ),
            },
            {
                "action_type": "tool",
                ...
                "purpose": "重新打开最终专业 Excel，核对结构和业务数据。",
            },
            {
                "action_type": "finish",
                "final_answer": (
                    "已完成城市销售分析并生成专业 Excel 报告；"
                    "最终报告已经重新读取检查。"
                ),
            },
        ]
...
def main():
    print("=" * 72)
    print("DataPilot v4.5 Professional Excel Agent E2E 确定性测试")
    print("=" * 72)

    with tempfile.TemporaryDirectory(
        prefix="datapilot_professional_excel_e2e_"
    ) as temp_dir:
        root = Path(temp_dir)
        ...
        user_task = (
            ...
            "包含销售冠军 KPI 和城市销售额对比图表。"
            "不要覆盖原文件，生成后重新检查最终 Excel。"
        )
        ...
        assert_true(
            ...
            "Professional Excel 要求没有进入 TaskPlan。",
        )
        print("PASS")
        ...
        assert_true(
            '"excel_report_delivery"' in selected_block,
            "真实 Agent 状态没有选中 excel_report_delivery。",
        )
        assert_true(
        ...
        assert_true(
            "excel_report_delivery" in system_prompt,
            "Prompt 缺少 excel_report_delivery Guidance。",
        )
        assert_true(
            "create_professional_excel_report"
            in system_prompt,
            "Prompt 缺少专业 Excel 创建 Tool Guidance。",
        )
        assert_true(
            "inspect_professional_excel_report"
            in system_prompt,
            "Prompt 缺少专业 Excel 检查 Tool Guidance。",
        )
        print("PASS")
        ...
        print("\n测试 7：Agent 自主路径真实调用 Professional Excel 创建 Tool")
        assert_true(
            "create_professional_excel_report"
            in tool_names,
            f"没有执行专业 Excel 创建 Tool：{tool_names}",
        )
        ...
        assert_true(
            ...
            "专业 Excel 创建 Tool 执行失败。",
        )
        print("PASS")

        print("\n测试 8：最终 Excel 位于 Workspace deliverables_dir")
        ...
        assert_true(
            ...
            f"最终专业 Excel 不存在：{deliverable_path}",
        )
        assert_true(
            deliverable_path.parent
            == deliverables_dir,
            "最终专业 Excel 没有写入 deliverables_dir。",
        )
        print("PASS")

        print("\n测试 9：真实 Excel 包含标题、KPI、业务表和原生图表")
        ...
        assert_true(
            ...
            f"最终 Excel 缺少城市销售汇总 Sheet：{workbook.sheetnames}",
        )
        ...
        assert_true(
            "销售冠军" in all_values
            and "澳门" in all_values,
            "最终 Excel 缺少销售冠军 KPI。",
        )
        print("PASS")

        print("\n测试 10：最终 Excel 业务数据正确")
        ...
        assert_true(
            ...
            "专业 Excel 最终检查 Tool 执行失败。",
        )

        sheet_info = next(
            item
            for item in inspection_output[
                "sheets"
            ]
            if item["sheet_name"]
            == "城市销售汇总"
        ...
        assert_true(
            actual == expected,
            f"最终 Excel 业务数据不正确：{actual}",
        )
        print("PASS")

        print("\n测试 11：专业 Excel 在最后一次写入后被重新检查")
        ...
        assert_true(
            ...
            in output_files,
            f"agent.py output_files 没有最终专业 Excel：{output_files}",
        )
        print("PASS")
        ...
        assert_true(
            ...
            "Professional Excel 接入破坏了 v4.0 plan 元数据。",
        )
        assert_true(
    ...
    print("\n" + "=" * 72)
    print("Professional Excel Agent E2E 测试通过！")
    print("=" * 72)
    print("已验证：")
    print("1. 自然语言 -> TaskPlan -> SkillSelector")
    print("2. excel_data_analysis + excel_report_delivery 组合 Guidance")
    print("3. 真实 ToolExecutor / ToolPreflight / ToolRegistry 执行边界")
    print("4. 真实读取 -> 分组统计 -> 排序")
    print("5. Professional Excel 创建 -> KPI / 图表 / 专业结构")
    print("6. 最终 Excel 写后重新检查")
